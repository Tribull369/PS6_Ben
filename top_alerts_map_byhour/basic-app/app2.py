...
def server(input, output, session):
    ...
    def top_alerts_plot():
        ...
        filtered_df_hour = collapsed_df_hour[
            ...
            (collapsed_df_hour['hour'] == selected_hour)
        ]
        aggregated_alerts = filtered_df_hour.groupby(['latitude_bin', 'longitude_bin'])['alert_count'].sum().reset_index()
        ...
        scatter_plot = alt.Chart(top_alerts).mark_circle(size=80, color='blue', opacity=0.7).encode(
            ...
            title=f'Top 10 Locations for {selected_type} - {selected_subtype} ({selected_hour}:00)',
            width=600,
            height=600
        )
